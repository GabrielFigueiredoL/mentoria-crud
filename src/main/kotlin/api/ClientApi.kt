package org.gabrielfigueiredol.api

import com.sun.net.httpserver.HttpExchange
import com.sun.net.httpserver.HttpServer
import kotlinx.serialization.json.Json
import org.gabrielfigueiredol.domain.Client
import org.gabrielfigueiredol.service.ClientService

class ClientApi(private val server: HttpServer) {
    private val path = "/clients"
    val clientService = ClientService()

    fun initClientApi() {
        server.createContext(path) { exchange ->
            when (exchange?.requestMethod) {
                "POST" -> register(exchange)
                "GET" -> getAll(exchange)
            }
        }
        server.createContext("$path/") {exchange ->
            val id = exchange.requestURI.path.removePrefix("$path/").toIntOrNull()
            if (id == null) {
                exchange.sendResponseHeaders(400, -1)
                exchange.close()
            } else {
                when (exchange?.requestMethod) {
                    "GET" -> getById(exchange, id)
                    "DELETE" -> deleteById(exchange, id)
                    "PUT" -> update(exchange, id)
                }
            }
        }
    }

    fun register(exchange: HttpExchange) {
        val json = exchange.requestBody.bufferedReader().readText()
        val client = Json.decodeFromString<Client>(json)
        val clientId = clientService.registerClient(client)
        val response = """
            {
                "id": $clientId
            }
        """.trimIndent()

        exchange.sendResponseHeaders(201, response.toByteArray().size.toLong())
        exchange.responseBody.write(response.toByteArray())
        exchange.close()
    }

    fun getAll(exchange: HttpExchange) {
        val clientList = clientService.getAllClients()
        val response = Json.encodeToString(clientList)
        exchange.sendResponseHeaders(200, response.toByteArray().size.toLong())
        exchange.responseBody.write(response.toByteArray())
        exchange.close()
    }

    fun getById(exchange: HttpExchange, id: Int) {
        val client: Client? = clientService.getClientById(id)
        if (client != null) {
            val response = Json.encodeToString(client)
            exchange.sendResponseHeaders(200, response.toByteArray().size.toLong())
            exchange.responseBody.write(response.toByteArray())
        } else {
            exchange.sendResponseHeaders(404, -1)
        }
        exchange.close()
    }

    fun deleteById(exchange: HttpExchange, id: Int) {
        val success = clientService.deleteClientById(id)
        if (success) {
            exchange.sendResponseHeaders(204, -1)
        } else {
            exchange.sendResponseHeaders(404, -1)
        }
        exchange.close()
    }

    fun update(exchange: HttpExchange, id: Int) {
        println("Entrou")
        val json = exchange.requestBody.bufferedReader().readText()
        println("json")
        val client = Json.decodeFromString<Client>(json)
        println("client")
        val success = clientService.updateClient(id, client)
        println("success")
        if (success) {
            exchange.sendResponseHeaders(200, -1)
        } else {
            exchange.sendResponseHeaders(404, -1)
        }
        exchange.close()
    }
}